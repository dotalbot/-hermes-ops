...
- [ ] Confirm the selected execution lane: the primary JellySSH route is `jellybase_jellyssh` through the OpenAI-Codex/Hermes worker on `jellydev@192.168.1.2`; the reviewed Claude adapter is an explicit optional exception and uses only `jellyclaude@jellybase` through the LAN `agent-claude` alias.
