...
> **Status:** Phase 1 completed and independently reviewable; awaiting operator acceptance
>
...
The OpenRouter credential is configured without exposing its value, and all named OpenRouter model identifiers were visible in the live model catalog. The preview UI model must be rechecked during Phase 2; disappearance or identifier drift blocks for operator selection rather than triggering fallback.
...
Final review: every implementation after all required reports
```
...
1. Create and verify the two project profiles only after Phase 2 authorization.
2. Prove a hard reviewer write boundary; prompt instructions are insufficient.
3. Reconcile stale hub lock provenance during selective promotion without losing local overlays.
4. Recheck exact OpenRouter model availability and credential usability in fresh profile sessions.
5. Verify the proposed Jellyberry/Jellybase paths, repository SSH authentication, ownership, clean state, remote, Flutter/Dart/Android tooling, and separate workspaces.
6. Verify `jellyssh-main` exists and test implementation/reviewer retention modes.
7. Implement no development and keep the proposed board without executable cards.
